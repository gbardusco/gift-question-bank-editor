...
const App: React.FC = () => {
  const { store, addCategory, editCategory, deleteCategory, moveCategory, saveQuestion, deleteQuestion, moveQuestion, bulkImport } = useQuestionStore();
  const { theme, toggleTheme } = useTheme();
  
  const [selectedCategoryId, setSelectedCategoryId] = useState<string | null>(store.categories[0]?.id || 'root');
      ...
      `}>
        <div className="flex flex-col h-full">
          ...
          <div className={`p-6 border-t flex flex-col gap-2 ${theme === 'dark' ? 'bg-slate-900/50 border-slate-800' : 'bg-slate-50 border-slate-100'}`}>
            ...
            <button onClick={() => downloadFile(exportToGift(store.categories, store.questions), 'banco.gift.txt')} className="w-full bg-indigo-600 text-white py-3 rounded-xl font-bold text-sm hover:bg-indigo-700 shadow-lg shadow-indigo-200 dark:shadow-none transition flex items-center justify-center gap-2">
              <Icons.Download /> Exportar Tudo
            </button>
          </div>
        </div>
      </aside>
