...
const App: React.FC = () => {
  ...
  return (
    <div className={`flex h-screen w-full overflow-hidden transition-colors duration-300 ${theme === 'dark' ? 'bg-slate-950 text-slate-100' : 'bg-slate-50 text-slate-900'}`}>
      ...
      {isSidebarOpen && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm z-40 lg:hidden" onClick={() => setIsSidebarOpen(false)} aria-hidden="true" />
      )}
      ...
      `} aria-label="Navegação Lateral">
        <div className="flex flex-col h-full">
          <div className={`p-6 border-b flex items-center justify-between ${theme === 'dark' ? 'border-slate-800' : 'border-slate-100'}`}>
            ...
            <button onClick={() => setIsSidebarOpen(false)} className="lg:hidden p-2 text-slate-400 hover:text-slate-600 transition" aria-label="Fechar menu">
            ...
            <div className="space-y-4 pt-4 border-t border-slate-100 dark:border-slate-800">
              <div className="flex items-center justify-between px-2">
                ...
                <div className="flex items-center gap-1">
                   <button onClick={expandAll} className="p-1 text-indigo-600 dark:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-indigo-900/30 rounded transition-colors" title="Expandir Tudo" aria-label="Expandir todas as categorias"><Icons.ExpandAll aria-hidden="true" /></button>
                   <button onClick={collapseAll} className="p-1 text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-800 rounded transition-colors" title="Recolher Tudo" aria-label="Recolher todas as categorias"><Icons.CollapseAll aria-hidden="true" /></button>
                   <div className="w-px h-3 bg-slate-200 dark:bg-slate-700 mx-1"></div>
                   <button onClick={() => setCatModal({ isOpen: true, mode: 'add', parentId: null, categoryId: null, name: '' })} className="p-1 text-indigo-600 dark:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-indigo-900/30 rounded flex items-center gap-1 transition-colors" title="Nova Categoria Raiz" aria-label="Adicionar categoria raiz">
                     <Icons.Plus aria-hidden="true" /> <span className="text-[9px] font-black uppercase">Root</span>
                   </button>
                </div>
              </div>
              <CategoryTree 
          ...
          <div className={`p-6 border-t flex flex-col gap-2 ${theme === 'dark' ? 'bg-slate-900/50 border-slate-800' : 'bg-slate-50 border-slate-100'}`}>
            <div className="space-y-1 mb-2 px-1">
              <label htmlFor="moodle-context" className="text-[9px] font-black uppercase text-slate-400 tracking-widest flex items-center gap-1">
                <i className="fas fa-cog" aria-hidden="true"></i> Contexto do Moodle
              </label>
              <select 
                id="moodle-context"
                value={exportContext} 
            ...
            <button onClick={() => setShowImport(true)} className={`w-full py-3 rounded-xl font-bold text-sm transition ${theme === 'dark' ? 'bg-slate-800 text-slate-300 border-slate-700 hover:bg-slate-700' : 'bg-white text-slate-700 border-slate-200 hover:bg-slate-100'}`} aria-label="Abrir modal de importação GIFT">
              <i className="fas fa-file-import mr-2" aria-hidden="true"></i>Importar GIFT
            </button>
            <button onClick={() => downloadFile(exportToGift(store.categories, store.questions, selectedCategoryId || undefined, exportContext), `${selectedCategory?.name || 'banco'}.gift.txt`)} className="w-full bg-indigo-600 text-white py-3 rounded-xl font-bold text-sm hover:bg-indigo-700 shadow-lg transition flex items-center justify-center gap-2" aria-label="Baixar arquivo GIFT">
              <Icons.Download aria-hidden="true" /> {selectedCategoryId === null || selectedCategoryId === 'root' ? 'Exportar Banco' : 'Exportar Categoria'}
            </button>
          </div>
        </div>
      </aside>
      ...
      <main className="flex-1 flex flex-col min-w-0" role="main">
        <header className={`px-4 sm:px-8 py-4 border-b flex items-center justify-between z-20 backdrop-blur-md sticky top-0 ${theme === 'dark' ? 'bg-slate-950/80 border-slate-800' : 'bg-white/80 border-slate-200'}`} aria-label="Barra de Ferramentas Principal">
          <div className="flex items-center gap-4 min-w-0">
            <button onClick={() => setIsSidebarOpen(!isSidebarOpen)} className="p-2 lg:hidden text-slate-500" aria-label="Alternar menu lateral"><i className="fas fa-bars" aria-hidden="true"></i></button>
            <div className="min-w-0">
              <nav className="hidden sm:flex text-[10px] font-bold text-slate-400 uppercase gap-2 mb-0.5" aria-label="Breadcrumb">
          ...
          <div className="flex items-center gap-3">
            <button onClick={toggleTheme} className={`p-2.5 rounded-xl transition ${theme === 'dark' ? 'bg-slate-800 text-amber-400 hover:bg-slate-700' : 'bg-slate-100 text-slate-500 hover:bg-slate-200'}`} aria-label={`Ativar modo ${theme === 'light' ? 'escuro' : 'claro'}`}><i className={`fas fa-${theme === 'light' ? 'moon' : 'sun'}`} aria-hidden="true"></i></button>
            <button onClick={handleNewQuestion} disabled={!selectedCategoryId} className="bg-emerald-600 text-white px-5 py-2.5 rounded-xl font-black shadow-lg shadow-emerald-200 dark:shadow-none active:scale-95 disabled:opacity-50 transition-all flex items-center gap-2" aria-label="Adicionar nova questão">
              <Icons.Plus aria-hidden="true" /> <span className="hidden sm:inline">Nova Questão</span>
            </button>
          </div>
        </header>
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-8 scrollbar-thin">
          {isEditingQuestion ? (
            <div className="max-w-5xl mx-auto">
              <button onClick={() => setIsEditingQuestion(false)} className="mb-6 text-indigo-600 dark:text-indigo-400 hover:underline flex items-center gap-2 font-bold uppercase text-[10px] tracking-widest" aria-label="Voltar para a listagem">
                <Icons.ArrowLeft aria-hidden="true" /> Cancelar e Voltar
              </button>
              <QuestionForm 
            ...
            <div className="max-w-6xl mx-auto pb-12">
              ...
              {filteredQuestions.length === 0 ? (
                <section className="flex flex-col items-center justify-center py-32 text-center opacity-80" aria-label="Estado Vazio">
                  <div className={`w-20 h-20 rounded-3xl flex items-center justify-center mb-6 ${theme === 'dark' ? 'bg-slate-900 text-slate-700' : 'bg-slate-100 text-slate-300'}`} aria-hidden="true"><i className="fas fa-ghost text-4xl"></i></div>
                ...
                <div className="grid grid-cols-1 gap-4" role="list" aria-label="Lista de Questões">
                  {filteredQuestions.map(q => (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-1 opacity-100 sm:opacity-0 group-hover:opacity-100 transition-opacity">
                        <button onClick={(e) => { e.stopPropagation(); handleDuplicateQuestion(q.id); }} className="p-3 text-slate-400 hover:text-emerald-500 hover:bg-emerald-50 dark:hover:bg-emerald-900/30 rounded-xl" title="Duplicar" aria-label={`Duplicar questão ${q.name}`}><Icons.Copy aria-hidden="true" /></button>
                        <button onClick={(e) => { e.stopPropagation(); handlePreviewQuestion(q); }} className="p-3 text-slate-400 hover:text-indigo-500 hover:bg-indigo-50 dark:hover:bg-indigo-900/30 rounded-xl" title="Simular Moodle" aria-label={`Simular visualização da questão ${q.name}`}><Icons.Search aria-hidden="true" /></button>
                        <button onClick={(e) => { e.stopPropagation(); handleSelectQuestion(q); }} className="p-3 text-slate-400 hover:text-indigo-500 hover:bg-indigo-50 dark:hover:bg-indigo-900/30 rounded-xl" aria-label={`Editar questão ${q.name}`}><Icons.Edit aria-hidden="true" /></button>
                        <button onClick={(e) => { e.stopPropagation(); handleConfirmDeleteQuestion(q.id); }} className="p-3 text-slate-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/30 rounded-xl" aria-label={`Excluir questão ${q.name}`}><Icons.Trash aria-hidden="true" /></button>
                      </div>
                    </article>
                  ))}
                </div>
      ...
      {bankModal.isOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/80 backdrop-blur-sm" role="dialog" aria-labelledby="bank-modal-title">
          <div className="bg-white dark:bg-slate-900 rounded-3xl w-full max-w-md shadow-2xl border border-slate-200 dark:border-slate-800 p-8 space-y-6 animate-in zoom-in duration-300">
            <h2 id="bank-modal-title" className="text-2xl font-black text-slate-800 dark:text-slate-100 uppercase tracking-tight">{bankModal.mode === 'add' ? 'Novo Banco' : 'Renomear Banco'}</h2>
            <form onSubmit={handleBankSubmit} className="space-y-6">
              <div className="space-y-2">
                <label htmlFor="bank-name" className="text-xs font-black uppercase text-slate-400 tracking-widest">Nome do Banco</label>
                <input id="bank-name" autoFocus value={bankModal.name} onChange={(e) => setBankModal(p => ({...p, name: e.target.value}))} className="w-full bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl p-4 outline-none text-slate-900 dark:text-slate-100 focus:ring-2 focus:ring-indigo-500 transition-all" required />
              </div>
      ...
      {showImport && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/70 backdrop-blur-sm" role="dialog" aria-labelledby="import-modal-title">
          <div className={`rounded-3xl w-full max-w-4xl shadow-2xl border p-8 space-y-4 animate-in zoom-in duration-300 ${theme === 'dark' ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'}`}>
            <div className="flex items-center justify-between">
              <h2 id="import-modal-title" className="text-2xl font-black uppercase tracking-tight">Importar GIFT</h2>
            ...
            <div onDragOver={(e) => { e.preventDefault(); setIsDraggingFile(true); }} onDragLeave={() => setIsDraggingFile(false)} onDrop={handleFileDrop} className={`relative group transition-all duration-300 rounded-2xl overflow-hidden border-2 border-dashed h-[450px] ${isDraggingFile ? 'border-indigo-500 bg-indigo-50 dark:bg-indigo-900/20' : 'border-slate-200 dark:border-slate-800'}`}>
              <div ref={highlightRef} className="absolute inset-0 p-6 font-mono text-xs whitespace-pre overflow-hidden pointer-events-none" aria-hidden="true">{renderHighlightedText()}</div>
              <textarea 
                ref={textareaRef} 
                onScroll={handleScroll} 
                className={`absolute inset-0 w-full h-full p-6 font-mono text-xs focus:ring-0 outline-none bg-transparent resize-none leading-[1.2rem] ${theme === 'dark' ? 'text-slate-200' : 'text-slate-800'}`} 
                placeholder="$CATEGORY: top/..." 
                aria-label="Editor de texto para importação GIFT"
                value={importText} 
                onChange={(e) => { setImportText(e.target.value); handleScroll(); }} 
              />
              {isDraggingFile && (
                <div className="absolute inset-0 pointer-events-none flex items-center justify-center bg-indigo-600/10 backdrop-blur-sm">
                  <div className="bg-white dark:bg-slate-900 px-6 py-3 rounded-xl shadow-xl font-black text-indigo-600 flex items-center gap-2"><i className="fas fa-file-upload text-xl" aria-hidden="true"></i> Solte o arquivo para importar</div>
                </div>
              )}
            </div>
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
              <div className="flex-1 min-w-0">
                {validationResults.length > 0 && (
                  <button onClick={() => setShowValidationReport(true)} className={`flex items-center gap-2 px-3 py-1.5 rounded-lg font-black text-[10px] transition-all transform hover:scale-105 uppercase tracking-widest ${hasErrors ? 'bg-red-500/10 text-red-500 hover:bg-red-500/20' : 'bg-amber-500/10 text-amber-500 hover:bg-amber-500/20'}`} aria-label={`Mostrar relatório de validação: ${errorCount} erros e ${warningCount} avisos`}>
                    <i className={`fas fa-${hasErrors ? 'times-circle' : 'exclamation-triangle'}`} aria-hidden="true"></i>
              ...
              <div className="flex gap-3 shrink-0">
                ...
                <button disabled={hasErrors} onClick={handleImport} className={`px-8 py-3 font-black rounded-xl transition active:scale-95 flex items-center gap-2 ${hasErrors ? 'bg-slate-200 dark:bg-slate-800 text-slate-400 cursor-not-allowed' : 'bg-emerald-600 text-white hover:bg-emerald-700 shadow-lg shadow-emerald-200 dark:shadow-none'}`} aria-label="Confirmar importação">
                  {hasErrors ? <i className="fas fa-lock" aria-hidden="true"></i> : <i className="fas fa-check" aria-hidden="true"></i>} Importar
                </button>
              </div>
            </div>
          </div>
      ...
      {showValidationReport && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-slate-900/90 backdrop-blur-md animate-in fade-in duration-300" role="alertdialog" aria-labelledby="report-modal-title">
          <div className={`rounded-3xl w-full max-w-2xl shadow-2xl border p-8 flex flex-col max-h-[80vh] ${theme === 'dark' ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'}`}>
            <div className="flex items-center justify-between mb-6">
              <h2 id="report-modal-title" className="text-2xl font-black flex items-center gap-3 uppercase tracking-tight"><i className="fas fa-clipboard-list text-indigo-500" aria-hidden="true"></i> Relatório de Erros</h2>
              <button onClick={() => setShowValidationReport(false)} className="text-slate-400 hover:text-slate-600 p-2" aria-label="Fechar relatório"><i className="fas fa-times text-xl" aria-hidden="true"></i></button>
            </div>
            <div className="flex-1 overflow-y-auto space-y-3 pr-2 scrollbar-thin">
              {validationResults.map((v, i) => (
                <div key={i} className={`p-4 rounded-2xl border flex items-start gap-4 transition-all ${v.severity === 'error' ? 'bg-red-50 dark:bg-red-900/10 border-red-200 dark:border-red-900/30' : 'bg-amber-50 dark:bg-amber-900/10 border-amber-200 dark:border-amber-900/30'}`}>
                  <div className={`mt-1 flex-shrink-0 w-10 h-10 rounded-xl flex items-center justify-center font-black text-xs ${v.severity === 'error' ? 'bg-red-500 text-white' : 'bg-amber-500 text-white'}`} aria-hidden="true">{v.severity === 'error' ? 'ERRO' : 'AVIS'}</div>
                  <div className="flex-1 min-w-0">
                    ...
                    <code className="block mt-2 p-2 bg-black/5 dark:bg-white/5 rounded-lg text-[10px] truncate italic" aria-hidden="true">"{v.text}"</code>
                  </div>
                </div>
              ))}
            </div>
